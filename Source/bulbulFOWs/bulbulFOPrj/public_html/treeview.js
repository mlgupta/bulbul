...
{ 
  ...
  this.morc=""              //Added by Sudheer -- Merchandise Only Flag
...
{ 
  ...
  if (browserVersion > 0) 
    auxEv = "<a class=\"tree\" href='javascript:clickOnNode(\""+this.getID()+"\")'>" 
  else 
    auxEv = "<a class=\"tree\" >" 
  ...
  {
	  if (supportsDeferral) {
		  doc.write("<div id='domRoot' width='100%'></div>") //transition between regular flow HTML, and node-insert DOM DHTML
...
{ 
  ...
  { 
	if (USEFRAMES)
	  docW = docW + "<a class=\"tree\" href='" + this.hreference + "' TARGET=\"basefrm\" "
	else
	  docW = docW + "<a class=\"tree\" href='" + this.hreference + "' TARGET=_top "
    ...
    if (browserVersion > 0) 
      docW = docW + "onClick='javascript:clickOnFolder(\""+this.getID()+"\",\"" + this.morc + "\")' "
    ...
    if (isTextLink)  
      docW += "id=\"itemTextLink"+this.id+"\" "
    docW += "onClick='javascript:clickOnFolder(\""+this.getID()+"\",\"" + this.morc + "\")' "
    docW += "onDblClick='javascript:clickOnNode(\""+this.getID()+"\",\"" + this.morc + "\")'" + ">"  
  }
  return docW;
} 
...
{ 
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=top>"
  if (USEICONS)
      docW = docW + "<a class=\"tree\" " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' border=0>" + "</a>"
  else
	  if (this.prependHTML == "")
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a class=\"tree\" " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
function clickOnFolder(folderId,morc) 
{ 
  ...
  //if (!(folder.isOpen4List)){                      
    if (PERSERVESTATE)
      SetCookie("clickedFolder4List", folder.getID());
    highlightObjLink(folder);
    userDefinedClickOnFolder(folderId,morc) //Added By Sudheer
  //}
...
{ 
  ...
  if ((folderObj.isOpen) )				
    if (!(folderObj.isNextLevel) ){
      userDefinedClickOnNode(folderId);
    }
}
...
// Below Code is Added By sudheer  to set state of the Folder Opened for List
  stateInCookie = GetCookie("clickedFolder4List");       
  if(stateInCookie!=null)
    if(stateInCookie!=""){
      nodeObj=findObj(stateInCookie);
      nodeObj.isOpen4List=true; 
