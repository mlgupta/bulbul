...
public class PaymentByDCAction extends Action  {
  /**
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    PaymentByDCForm pform=null;
    String paymentMode=null;
    try{
      logger.info("Entering PaymentByDCAction");
      pform = (PaymentByDCForm)form;
      paymentMode=Integer.toString( pform.getHdnPaymentMode());
      request.setAttribute("paymentMode",paymentMode);
    }catch(Exception e){
      logger.error(e);
    }finally{
      logger.info("Exiting PaymentByDCAction");
    }
