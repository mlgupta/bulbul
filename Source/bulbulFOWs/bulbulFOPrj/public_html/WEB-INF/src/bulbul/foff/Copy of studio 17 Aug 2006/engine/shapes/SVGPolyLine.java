...
public class SVGPolyLine extends SVGShape  {
  ...
  private String shapepolylinelabel="Polyline";

  private String shapepolylineundoredocreate="Create Polyline";
  private String shapepolylineundoredotranslate="Translate Polyline";
  private String shapepolylineundoredoresize="Resize Polyline";
  private String shapepolylineundoredorotate="Rotate Polyline";
  private String shapepolylineundoredomodifypoint="Modify Point";

  private String shapepolylinehelpcreate="Draw a polyline";

  private final SVGPolyLine  svgPolyLine=this;
  private DecimalFormat  format=null;
  private PolyLineActionListener polyLineAction=null;
   ...
   * @param studio
   */
  public SVGPolyLine(Studio studio) {
    super(studio);
    ids.put("id","polyline");
    
    try{
      labels.put("label", shapepolylinelabel);
      labels.put("undoredocreate", shapepolylineundoredocreate);
      labels.put("undoredotranslate", shapepolylineundoredotranslate);
      labels.put("undoredoresize", shapepolylineundoredoresize);
      labels.put("undoredomodifypoint", shapepolylineundoredomodifypoint);
      labels.put("undoredorotate", shapepolylineundoredorotate);
      labels.put("helpcreate", shapepolylinehelpcreate);
    }catch (Exception ex){}
    
    DecimalFormatSymbols symbols=new DecimalFormatSymbols();
		symbols.setDecimalSeparator('.');
		format=new DecimalFormat("######.#",symbols);
    polyLineAction=new PolyLineActionListener();
    //a listener that listens to the changes of the SVGFrames
		final ActionListener svgTabChangedListener=new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				polyLineAction.reset();
			}
		};
		
		//adds the SVGTab change listener
		getStudio().getSvgTabManager().addSVGTabChangedListener(svgTabChangedListener);
    svgToolItem=getStudio().getSvgToolBar().getStbPolyLine();
    svgToolItem.addActionListener(polyLineAction);
  }
    ...
    if(svgTab!=null && points!=null && points.length>1){
			...
			if(getStudio().getSvgSelection()!=null && doc!=null){
				...
				if(parent!=null){
					...
					final Element polyline=doc.createElementNS(doc.getDocumentElement().getNamespaceURI(),"polyline");

					polyline.setAttributeNS(null,"points",value);
					String colorString=getStudio().getColorChooser().getColorString(getStudio().getSvgColorManager().getCurrentColor());
					polyline.setAttributeNS(null, "style", "stroke:".concat(colorString.concat(";fill:none;")));
					...
					if(getStudio().getSvgUndoRedo()!=null){
						...
						if(selection!=null){
							...
							selection.handleNodeSelection(svgTab, polyline);
  ...
  protected void drawGhost(SVGTab svgTab, Graphics2D g2D, Point2D.Double[] points){
   if(svgTab!=null && points!=null && points.length>1 && g2D!=null){
			//computes and draws the new awt polyline to be displayed
      ...
      int squareDistance=5;
        ...
        //removes the first space characters from the string
				...
				if(points!=null && points.length>0){
					...
					BridgeContext ctxt=svgTab.getScrollPane().getSvgCanvas().getUpdateManager().getBridgeContext();
					if(ctxt!=null){
						GraphicsNode gnode=ctxt.getGraphicsNode(elementNode);
						if(gnode!=null){
  ...
  public void modifyPoint(SVGTab svgTab, SVGSelectionSquare square, Point point1, Point point2){
    ...
    if(svgTab!=null && square!=null && square.getNode()!=null && square.getNode() instanceof Element && point2!=null){
			...
			try{paintListener=(
        SVGPaintListener)modifyPointSvgTabTable.get(svgTab);
        ...
        double py=0;
          ...
          endPath.closePath();
					...
					af.preConcatenate(AffineTransform.getTranslateInstance(translateValues.width, translateValues.height));

					//computing the outline
					final Shape outline=af.createTransformedShape(endPath);
    ...
    if(svgTab!=null && square!=null && square.getNode()!=null && square.getNode() instanceof Element && point2!=null){
        ...
        //removes the first space characters from the string
              ...
              pointsString=pointsString.replaceFirst("[\\s]+","");
						...
						listOfPoints.add(new Point2D.Double(px, py));
					}catch (Exception ex){break;}
				}
				...
				//the path will be displayed
				GeneralPath path=new GeneralPath();
				Point2D.Double pt0=(Point2D.Double)listOfPoints.get(0);
        Point2D.Double pt=null;
          ...
          pt=(Point2D.Double)listOfPoints.get(i);
          path.lineTo((float)pt.x, (float)pt.y);
				}
  ...
  public void cancelActions(){
    if(polyLineAction!=null)polyLineAction.cancelActions();
  }
   ...
   * @author Sudheer V. Pujar
   */
  protected class PolyLineActionListener implements ActionListener{
   
    private final Hashtable  mouseAdapterSvgTabs=new Hashtable();
    private final PolyLineActionListener action=this;
     ...
     * @description 
     */
    protected PolyLineActionListener(){
      createCursor=getStudio().getSvgCursors().getCursor("polyline");
    }
    ...
    protected void reset(){
      if(isActive){
            ...
            mouseAdapterSvgTabs.remove(it.next());
				...
				PolyLineMouseListener polyLineMouseListener=null;
            ...
            svgTab=(SVGTab)it.next();
					...
					if(svgTab!=null && ! mouseAdapterSvgTabs.containsKey(svgTab)){
						polyLineMouseListener=new PolyLineMouseListener(svgTab);
						try{
							svgTab.getScrollPane().getSvgCanvas().addMouseListener(polyLineMouseListener);
							svgTab.getScrollPane().getSvgCanvas().addMouseMotionListener(polyLineMouseListener);
							svgTab.getScrollPane().getSvgCanvas().setSVGCursor(createCursor);
						}catch (Exception ex){}
						mouseAdapterSvgTabs.put(svgTab, polyLineMouseListener);
					}
				}				
			}
    }
            ...
            svgTab=(SVGTab)it.next();
					...
					if(svgTab!=null){
						...
						try{
							...
							svgTab.getScrollPane().getSvgCanvas().removeMouseListener((MouseAdapter)mouseListener);
							svgTab.getScrollPane().getSvgCanvas().removeMouseMotionListener((MouseMotionListener)mouseListener);
							if(mouseListener!=null && ((PolyLineMouseListener)mouseListener).paintListener!=null){
								//removes the paint listener
								svgTab.getScrollPane().getSvgCanvas().removePaintListener(((PolyLineMouseListener)mouseListener).paintListener, true);
							}
						}catch (Exception ex){}
						toBeRemoved.add(svgTab);
					}
   ...
   * @author Sudheer V. Pujar
   */
  protected class PolyLineMouseListener extends MouseAdapter implements MouseMotionListener{
     ...
     * @param svgTab
     */
    public PolyLineMouseListener(SVGTab svgTab){
      this.svgTab=svgTab;
      ...
      paintListener=new SVGPaintListener(){
        public void paintToBeDone(Graphics g) {
          if(points.size()>1){
            //draws the shape of the polyline that will be created if the user released the mouse button
            ...
            if(g!=null && scaledPoints!=null){
              svgPolyLine.drawGhost(finalSvgTab, (Graphics2D)g, scaledPoints);
            }
          }
        }
      };
    ...
    public void mouseMoved(MouseEvent evt){
      ...
      //asks the canvas to be repainted to draw the shape of the future polyline
    ...
    public void mouseClicked(MouseEvent evt) {
      ...
      if(point!=null){
        ...
        if(isDoubleClick){
          if(points.size()>0){
            //creates the polyline in the SVG document
            ...
            if(queue.getThread()!=Thread.currentThread()){
              queue.invokeLater(new Runnable(){
                public void run(){
                  svgPolyLine.drawPolyline(svgTab, finalPoints);
                }
              });
            }else{
              svgPolyLine.drawPolyline(svgTab, finalPoints);
