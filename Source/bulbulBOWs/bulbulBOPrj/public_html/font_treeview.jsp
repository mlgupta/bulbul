...
<bean:define id="jsFileLinks" name="fontTreeView" property="jsFileLinks" />
<bean:define id="jsFileName" name="fontTreeView" property="jsFileName" />
<bean:define id="treeOperation" name="treeOperation"  />

<html>
<head>
<title>Font Tree View</title>
...
function userDefinedClickOnFolder(folderId){
  window.top.location.replace("fontRelayAction.do?hdnSearchOperation=font_list&fontOrCategoryTblPk="+ folderId  );		     
}

function userDefinedClickOnNode(folderId){
  window.location.replace('fontRelayAction.do?hdnSearchOperation=font_tree_view_append&selectedCategoryPk='+ folderId + '&currentCategoryPk=<%=currentCategoryPk%>'  );	
}
