...
<bean:define id="pageCount" name="printableAreaListForm" property="hdnSearchPageCount" />        
<bean:define id="pageNo" name="printableAreaListForm" property="hdnSearchPageNo" />        
<bean:define id="printableAreas" name="printableAreas" type="java.util.ArrayList" /> 
<bean:define id="userRights" name="userProfile" property="userRights" />        
<HTML>
  <HEAD>  
    <TITLE><bean:message  key="title.PrintableAreaList" /></TITLE>
    ...
    <script language="JavaScript" type="text/JavaScript"  >
      ...
      function btnclick(operation){
        ...
        if (operation!='page' && operation!='search' && operation!='add' && operation!='priority'){
      ...
      function btnSearchCloseOnclick(){
        var thisForm=document.forms[0];
        thisForm.txtSearchPrintableAreaName.value='';
...
<html:form action="/printableAreaRelayAction.do">
        ...
        <tr>
          <td width="87%" height="55px" valign="bottom">
            ...
            <table width="100%"  border="0" class="bdrColor_336666" id="tblSearch" style="display:none">
              <tr>
                <td width="8%">
                  <div align="right"><bean:message  key="txt.PrintableAreaName" />: </div>
                </td>
                <td width="11%">
                  <html:text property="txtSearchPrintableAreaName"  styleClass="bdrColor_336666" style="width:100px" tabindex="2"/>
                </td>
                  ...
                  <logic:iterate id="module" name="userRights">          
                    <logic:equal name="module" property="name" value="Printable Area">
                      <logic:iterate id="permission" name="module" property="permissions">  
                        ...
                        <!--Priority Button Enable Disable-->
                        <logic:equal name="permission" property="name" value="Priority">
                          <logic:equal name="permission" property="value" value="1">
                            <html:button property="btnPriority" styleClass="buttons" tabindex="18" onclick="btnclick('priority')" ><bean:message  key="btn.Priority" /></html:button>
                          </logic:equal>
                          <logic:equal name="permission" property="value" value="0">
                            <html:button property="btnPriority" styleClass="buttonDisabled" tabindex="18" disabled="true"><bean:message  key="btn.Priority" /></html:button>
                          </logic:equal>
                        </logic:equal>
                      </logic:iterate>                          
                    </logic:equal>
    ...
    <td align="center">
      ...
      <div style="overflow:auto;height:397px;">
        <table id="tblList"  width="98%"  border="0" cellspacing="1" bgcolor="#80A0B2">
          <tr bgcolor="#FFFFFF">
            <th width="3%">&nbsp;</th>
            <th width="25%"><bean:message  key="tbl.Name" /></th>
            <th width="17%"><bean:message  key="tbl.Status" /></th>
            <th width="7%"><bean:message   key="tbl.Priority"/></th>
          </tr>
          <%if(printableAreas.size()>0){ %>
            <logic:iterate id="printableArea" name="printableAreas">  
              <bean:define id="PrintableAreaTblPk" name="printableArea" property="printableAreaTblPk" />
              <tr bgcolor="#FFFFFF"> 
                <td><html:radio property="radSearchSelect" tabindex="18" onclick="return radioOnlick(this);" value="<%=(String)PrintableAreaTblPk %>"></html:radio></td>
                <td><bean:write name="printableArea" property="printableAreaName" /></td>
                <td><bean:write name="printableArea" property="isActiveDisplay" />
                <td><bean:write name="printableArea" property="printableAreaPriority" />
                <bean:define id="intStatus" name="printableArea" property="isActive" />
