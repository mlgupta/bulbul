...
public class PrintableAreaDeleteAction extends Action  {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    logger.info("Entering PrintableArea Delete Action");
    try{

      Operations.deletePrintableArea(printableAreaTblPk,dataSource);  
      ...
      if((e.getMessage().indexOf(ErrorConstants.REFERED.getErrorValue()))>-1){
        try{
          printableAreaId=Operations.getPrintableAreaName(printableAreaTblPk,dataSource);
        }catch(Exception ex){
          logger.error("Error getPrintableAreaId"+ex);
        }
        
        ActionMessage msg=new ActionMessage("msg.Refered",printableAreaId);
      ...
      logger.info("Exiting PrintableArea Delete Action");
    }
